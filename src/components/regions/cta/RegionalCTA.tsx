...
interface RegionalCTAProps {
  regionName: string;
  // Add region prop to match usage in RegionalPageTemplate.tsx
  region?: string;
}

const RegionalCTA: React.FC<RegionalCTAProps> = ({ 
  regionName,
  region, 
}) => {
  // Use region as regionName if provided
  const displayRegion = region || regionName;
  
  return (
    <section className="py-16 bg-gray-50">
      <div className="container mx-auto px-4 text-center">
        <h2 className="text-3xl font-serif font-semibold text-wrap-blue mb-4">
          Ready to Transform Your Vehicles in {displayRegion}?
        </h2>
        <p className="text-wrap-grey text-lg mb-8 max-w-2xl mx-auto">
          Contact our team today for a free consultation and quote on premium vehicle wrapping, ceramic coating, and paint protection services throughout {displayRegion}.
        </p>
